/*
* This header is generated by classdump-dyld 0.7
* on Wednesday, June 24, 2015 at 11:17:55 PM Japan Standard Time
* Operating System: Version 8.3 (Build 12F69)
* Image Source: /System/Library/Frameworks/UIKit.framework/UIKit
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <UIKit/UIKit-Structs.h>
#import <UIKit/UICollectionViewLayoutAttributes.h>

@interface _UIAlertControllerCollectionViewLayoutAttributes : UICollectionViewLayoutAttributes {

	float _alignedDescriptiveLabelTextWidth;

}

@property (assign) float alignedDescriptiveLabelTextWidth;              //@synthesize alignedDescriptiveLabelTextWidth=_alignedDescriptiveLabelTextWidth - In the implementation block
-(char)isEqual:(id)arg1 ;
-(id)copyWithZone:(NSZone*)arg1 ;
-(void)setAlignedDescriptiveLabelTextWidth:(float)arg1 ;
-(float)alignedDescriptiveLabelTextWidth;
@end

