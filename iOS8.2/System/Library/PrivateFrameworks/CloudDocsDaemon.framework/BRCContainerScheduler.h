/*
* This header is generated by classdump-dyld 0.7
* on Tuesday, June 23, 2015 at 11:19:41 PM Japan Standard Time
* Operating System: Version 8.2 (Build 12D508)
* Image Source: /System/Library/PrivateFrameworks/CloudDocsDaemon.framework/CloudDocsDaemon
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <CloudDocsDaemon/CloudDocsDaemon-Structs.h>
#import <CloudDocsDaemon/APSConnectionDelegate.h>
#import <CloudDocsDaemon/BRCLocalContainerDelegate.h>
#import <CloudDocsDaemon/BRCLowDiskDelegate.h>

@protocol OS_dispatch_queue, OS_dispatch_source, OS_dispatch_group;
@class BRCAccountSession, NSMutableDictionary, BRCRelativePath, PQLConnection, BRCCountedSet, BRCMinHeap, NSObject, NSOperationQueue, CKContainer, NSString, APSConnection, BRCContainerMetadataSyncPersistedState, BRCOperation, BRCThrottle, BRCThrottledScheduler, BRCSyncBudgetThrottle;

@interface BRCContainerScheduler : NSObject <APSConnectionDelegate, BRCLocalContainerDelegate, BRCLowDiskDelegate> {

	BRCAccountSession* _session;
	opaque_pthread_rwlock_t _lock;
	NSMutableDictionary* _localContainersByID;
	NSMutableDictionary* _localContainersByMangledID;
	NSMutableDictionary* _serverContainersByID;
	BRCRelativePath* _root;
	PQLConnection* _clientDB;
	PQLConnection* _serverDB;
	BRCCountedSet* _coordinatedReaders;
	char _readerCountReachedMax;
	BRCMinHeap* _lostHeap;
	NSObject*<OS_dispatch_queue> _lostScanQueue;
	NSObject*<OS_dispatch_source> _lostScanSource;
	NSObject*<OS_dispatch_source> _lostScanDelay;
	/*^block*/id _lostScanHandler;
	BRCCountedSet* _coordinatedWriters;
	char _applyCountReachedMax;
	BRCMinHeap* _syncHeap;
	NSObject*<OS_dispatch_queue> _syncQueue;
	NSObject*<OS_dispatch_source> _syncSource;
	NSObject*<OS_dispatch_source> _syncDelay;
	NSOperationQueue* _syncOperationQueue;
	NSObject*<OS_dispatch_source> _containersMetadataSyncSource;
	CKContainer* _ckContainerForContainersMetadataSync;
	NSObject*<OS_dispatch_source> _pushSource;
	NSString* _environmentName;
	APSConnection* _pushConnection;
	NSObject*<OS_dispatch_queue> _pushQueue;
	Ai _uploadSuspendCount;
	Ai _downloadSuspendCount;
	BRCContainerMetadataSyncPersistedState* _containerMetadataPersistedState;
	unsigned _containerMetadataSyncState;
	BRCOperation* _containerMetadataSyncOperation;
	BRCThrottle* _containerResetThrottle;
	unsigned long long _availableQuota;
	char _isInLowDisk;
	BRCThrottledScheduler* _readerScheduler;
	BRCThrottledScheduler* _applyChangesScheduler;
	NSObject*<OS_dispatch_group> _lostScanGroup;
	NSObject*<OS_dispatch_group> _writerGroup;
	NSObject*<OS_dispatch_group> _uploadGroup;
	NSObject*<OS_dispatch_group> _downloadGroup;
	NSObject*<OS_dispatch_group> _initialSyncDownGroup;
	NSObject*<OS_dispatch_group> _syncGroup;
	BRCSyncBudgetThrottle* _syncUpBudget;

}

@property (nonatomic,readonly) BRCAccountSession * accountSession;                             //@synthesize session=_session - In the implementation block
@property (assign,nonatomic) unsigned long long availableQuota;                                //@synthesize availableQuota=_availableQuota - In the implementation block
@property (nonatomic,readonly) BRCThrottledScheduler * readerScheduler;                        //@synthesize readerScheduler=_readerScheduler - In the implementation block
@property (nonatomic,readonly) NSObject*<OS_dispatch_group> lostScanGroup;                     //@synthesize lostScanGroup=_lostScanGroup - In the implementation block
@property (nonatomic,readonly) NSObject*<OS_dispatch_group> writerGroup;                       //@synthesize writerGroup=_writerGroup - In the implementation block
@property (nonatomic,readonly) BRCThrottledScheduler * applyChangesScheduler;                  //@synthesize applyChangesScheduler=_applyChangesScheduler - In the implementation block
@property (nonatomic,readonly) NSObject*<OS_dispatch_group> initialSyncDownGroup;              //@synthesize initialSyncDownGroup=_initialSyncDownGroup - In the implementation block
@property (nonatomic,readonly) NSObject*<OS_dispatch_group> syncGroup;                         //@synthesize syncGroup=_syncGroup - In the implementation block
@property (nonatomic,readonly) BRCSyncBudgetThrottle * syncUpBudget;                           //@synthesize syncUpBudget=_syncUpBudget - In the implementation block
@property (nonatomic,readonly) NSObject*<OS_dispatch_group> uploadGroup;                       //@synthesize uploadGroup=_uploadGroup - In the implementation block
@property (nonatomic,readonly) NSObject*<OS_dispatch_group> downloadGroup;                     //@synthesize downloadGroup=_downloadGroup - In the implementation block
@property (readonly) unsigned hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(void)deactivate;
-(void)close;
-(BRCAccountSession *)accountSession;
-(id)containerByID:(id)arg1 ;
-(void)enumerateContainers:(/*^block*/id)arg1 ;
-(void)dumpToContext:(id)arg1 ;
-(id)initWithAccountSession:(id)arg1 ;
-(BRCThrottledScheduler *)readerScheduler;
-(BRCThrottledScheduler *)applyChangesScheduler;
-(id)containerByMangledID:(id)arg1 ;
-(long long)_readerScheduleContainer:(id)arg1 ;
-(void)_lostScanSchedule;
-(long long)_applyChangesScheduleContainer:(id)arg1 ;
-(void)_syncSchedule;
-(void)_syncScheduleForContainersMetadata;
-(void)_updatePushTopicsRegistration;
-(void)_automaticSyncSchedule;
-(id)createContainerIfNeeded:(id)arg1 isInReset:(char)arg2 ;
-(void)resetContainer:(id)arg1 resetType:(unsigned)arg2 ;
-(void)scheduleReset:(unsigned)arg1 forContainer:(id)arg2 ;
-(void)_removeContainerFromSyncList:(id)arg1 ;
-(void)endReadCoordinationInContainer:(id)arg1 ;
-(void)endWriteCoordinationInContainer:(id)arg1 ;
-(void)_removeContainerFromLostScanList:(id)arg1 ;
-(void)_unscheduleContainer:(id)arg1 ;
-(id)removeAndReturnLocalContainers;
-(void)activateLocalContainer:(id)arg1 withServerContainer:(id)arg2 creation:(char)arg3 ;
-(void)_scheduleUpdatePushTopicsRegistration;
-(void)_signalContainersMetadataSync;
-(void)_addContainerToScheduler:(id)arg1 ;
-(char)createContainerOnDisk:(id)arg1 createdRoot:(char*)arg2 createdDocuments:(char*)arg3 ;
-(void)resetContainerAsync:(id)arg1 ;
-(unsigned)_readCoordinationCount;
-(unsigned)_writeCoordinationCount;
-(long long)_applyAdditionsInContainer:(id)arg1 ;
-(id)ckContainerForContainersMetadataSync;
-(id)_containersMetadataSyncDatabase;
-(void)scheduleSyncDownContainerMetadata;
-(void)downloadSuspend;
-(void)downloadResume;
-(void)connection:(id)arg1 didReceivePublicToken:(id)arg2 ;
-(void)connection:(id)arg1 didReceiveToken:(id)arg2 forTopic:(id)arg3 identifier:(id)arg4 ;
-(void)connection:(id)arg1 didReceiveIncomingMessage:(id)arg2 ;
-(void)didChangeLostScanStatusForContainer:(id)arg1 ;
-(void)didChangeReaderStatusForContainer:(id)arg1 ;
-(void)didChangeSyncStatusForContainer:(id)arg1 ;
-(void)didChangeSyncStatusForContainerMetadataForContainer:(id)arg1 ;
-(void)didChangeApplyChangesStatusForContainer:(id)arg1 ;
-(void)didMarkItemNeedsDownloadInContainer:(id)arg1 oldState:(unsigned)arg2 newState:(unsigned)arg3 ;
-(void)didMarkItemNeedsUploadInContainer:(id)arg1 oldState:(unsigned)arg2 newState:(unsigned)arg3 ;
-(char)startReadCoordinationInContainer:(id)arg1 ;
-(char)startWriteCoordinationInContainer:(id)arg1 ;
-(void)willInitialSyncDownForContainer:(id)arg1 ;
-(void)didInitialSyncDownForContainer:(id)arg1 ;
-(void)containerDidBecomeForeground:(id)arg1 ;
-(void)containerDidBecomeBackground:(id)arg1 ;
-(void)lowDiskStatusChangedForDevice:(int)arg1 hasEnoughSpace:(char)arg2 ;
-(void)setupWithRoot:(id)arg1 ;
-(void)refreshPushRegistrationAfterAppsListChanged;
-(void)setAvailableQuota:(unsigned long long)arg1 ;
-(void)lostScanSetupTargetQueue:(id)arg1 handler:(/*^block*/id)arg2 ;
-(void)lostScanSuspend;
-(void)lostScanResume;
-(void)syncSuspend;
-(void)syncResume;
-(void)uploadSuspend;
-(void)uploadResume;
-(NSObject*<OS_dispatch_group>)lostScanGroup;
-(NSObject*<OS_dispatch_group>)writerGroup;
-(NSObject*<OS_dispatch_group>)uploadGroup;
-(NSObject*<OS_dispatch_group>)downloadGroup;
-(NSObject*<OS_dispatch_group>)initialSyncDownGroup;
-(NSObject*<OS_dispatch_group>)syncGroup;
-(BRCSyncBudgetThrottle *)syncUpBudget;
-(unsigned long long)availableQuota;
-(id)_containerMetadataRecordsToSave;
@end

